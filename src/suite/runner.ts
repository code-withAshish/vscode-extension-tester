  ...
  runTests(
    ...
    return new Promise((resolve) => {
      ...
      for (const pattern of testFilesPattern) {
        ...
        glob.sync(universalPattern).forEach((val) => testFiles.add(val));
      }
      ...
      this.mocha.suite.beforeAll(async function () {
        ...
        const binPath =
          process.platform === "darwin"
            ? await self.createShortcut(code.getCodeFolder(), self.tmpLink)
            : self.chromeBin;
        await browser.start(binPath, { headless: true });
  ...
  private loadConfig(config?: string): Mocha.MochaOptions {
    ...
    if (file && fs.existsSync(file) && fs.statSync(file).isFile()) {
      ...
      if (/\.(yml|yaml)$/.test(file)) {
        ...
        console.log(
          "Unsupported mocha configuration file extension, make sure to use .js, .json, .yml or .yaml file"
        );
      }
    }
    return conf;
